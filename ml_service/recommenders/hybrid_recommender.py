...
class HybridRecommender:
    ...
    def recommend(self, user_id, watch_history, top_n=10):
        ...
        if(num_watched_movies == 0): # No watched movies -> return most popular movies
            return dict(sorted(popularity_recom.items(), key=lambda x: x[1], reverse=True)[:top_n])
            ...
            for movie_id in recom_movie_ids:
                recoms[movie_id] = W_HYBRID_RECOM["w_pop"] * popularity_recom.get(movie_id, 0) + W_HYBRID_RECOM["w_con"] * content_recom.get(movie_id, 0) + W_HYBRID_RECOM["w_collab"] * collaborative_recom.get(movie_id, 0)
            return dict(sorted(recoms.items(), key=lambda x: x[1], reverse=True)[:top_n])
